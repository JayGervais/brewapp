...
{
    [Authorize]
    [Route("api/users/{user_Id}/photos")]
    [ApiController]
    public class PhotosController : ControllerBase
    {
        ...
        public PhotosController(IBrewRepository repo, IMapper mapper, 
        ...
        [HttpPost]
        public async Task<IActionResult> AddPhotoForUser(int user_Id, [FromForm]PhotoForCreationDto photoForCreationDto)
        {
            if (user_Id != int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value))
            {
                return Unauthorized();
            }
            var userFromRepo = await _repo.GetUser(user_Id);
        ...
        [HttpPost("{id}/setMain")]
        public async Task<IActionResult> SetMainPhoto(int user_Id, int id)
        {
            if (user_Id != int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value))
            {
                return Unauthorized();
            ...
            var currentMainPhoto = await _repo.GetMainPhotoForUser(user_Id);
